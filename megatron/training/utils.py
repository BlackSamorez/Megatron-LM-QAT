...
def get_top_logits_batch(current_seq_counter: int, seqs_to_consume_per_dp: int):
    ...
    def _broadcast(item):
       if item is not None:
            # if mpu.get_tensor_model_parallel_src_rank() == mpu.get_tensor_model_parallel_rank():
            #     logger.warning(f"Broadcasting shape {item.shape} dtype {item.dtype} from rank {mpu.get_tensor_model_parallel_src_rank()}")
            # else:
            #     logger.warning(f"Awaiting shape {item.shape} dtype {item.dtype} on tp rank {mpu.get_tensor_model_parallel_rank()} from {mpu.get_tensor_model_parallel_src_rank()}")
            torch.distributed.broadcast(item, mpu.get_tensor_model_parallel_src_rank(), group=mpu.get_tensor_model_parallel_group())
